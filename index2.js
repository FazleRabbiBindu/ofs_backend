...
async function run() {
    try {
        ...
        const cardsCollection = client.db('Ofs').collection('cards');
        ...
        app.delete('/delvendor/:id', async (req, res) => {
            ...
            res.send(result);
                ...
                app.post('/addcards', upload.single('pdffile'), async (req, res) => {
                    try {
                        const { pdfname, imagelink, title, animation } = req.body; // Get other fields from the body
                        const fileLocation = req.file.path.replace(/\\/g, '/'); // Normalize file path
                
                        // Create newCertificate object with all fields
                        ...
                        const result = await cardsCollection.insertOne(newCertificate);
                
                        // Send back the inserted ID and file location
                ...
                app.delete('/delcards/:id', async (req, res) => {
                    const id = req.params.id;
                    const query = { _id: new ObjectId(id) };
                    console.log('delete: ');
                ...
                app.get('/cards/:filename', async (req, res) => {
                    const filename = req.params.filename; // Get the filename from the URL
                    try {
                        const pdfDocument = await cardsCollection.findOne({ fileLocation: { $regex: filename } });
